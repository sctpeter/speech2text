...
* `course/`：用于存放原始未处理的上课视频或音频（支持 `.mp4`, `.mp3`, `.m4a`）。
...
## 注意与限制
- **仅预设 `.mp4`/`.mp3`/`.m4a` 支持**：当前仅扫描 `.mp4`、`.mp3` 与 `.m4a`，以避免误处理其他文件。底层 `faster-whisper` 与 `ffmpeg` 支持 `.mkv`、`.wav`、`.flac` 等多种格式，如有需要可调整 `run_course.sh` 与 `delete.sh` 的通配符。
...
**运行过程：**
1. 终端提示输入下载链接与课程名称。
2. 从链接解析日期（若无则使用当天日期），并依据链接后缀自动选择 `.mp4`/`.mp3`/`.m4a`（无法识别时回退为 `.mp4`），生成如 `马原_20260305.mp4` 的文件名并使用 `aria2` 下载。
...
```
**运行过程：**
1. 终端反复提示输入链接与课程名称，在链接处输入 `done` 结束收集。
2. 批量使用 `aria2` 下载全部课程至 `course/`。
3. 依次完成转录与 AI 精炼，统一输出至 `result/`。

> 网络问题需用户自行处理。最终结果在 `result/` 目录中生成 `.md` 文件。

---

### 方案 B：本地已有视频 — 无需下载

将待处理文件放入 `course/` 目录后，可选择以下两种方式：

#### B1：一键转录 + 精炼
```bash
bash transcribe_and_refinement.sh
```
*(等同于依次执行 B2 的两个步骤，适合一次性处理)*
